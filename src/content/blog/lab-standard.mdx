...
The file that makes this about reproducibility and not just tidiness is `manifest.json`. Every run writes one automatically. It records the git commit of the code (plus any uncommitted changes, as a diff), the exact conda environment, the random seed, the fully resolved config, a sha256 hash of every input and output file, and the machine and SLURM job it ran on.
